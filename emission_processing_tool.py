    ...
    for category in selected_categories:
        ...
        try:
            with rasterio.open(geotiff_path) as src:
                ...
                # Transpose and flip to ensure north-up
                emissions_resampled = emissions_resampled.transpose(0, 2, 1)[:, ::-1, :]  # Rotate 90° left and flip y-axis
...
# Adjust indices for transposed and flipped orientation
j, i = np.where(has_emission)  # Swap i and j to match transposed array
i = nx - 1 - i  # Flip i indices to ensure north-up
vsrc_i, vsrc_j, vsrc_k = i, j, np.ones_like(i, dtype=np.int32)
print(f"Number of source locations: {len(vsrc_i)}")
print(f"Sample source locations: {list(zip(vsrc_i[:5], vsrc_j[:5], vsrc_k[:5]))}")
vsrc_species = {f"vsrc_{species}": data[:, vsrc_i, vsrc_j] for species, (data, _) in vsrc_data.items()}
...
with nc.Dataset(output_path, 'w', format='NETCDF3_CLASSIC') as ds:
    ...
    # Create coordinate variables (swap and flip x for north-up)
    x_var = ds.createVariable('x', 'f4', ('x',))  # Vertical axis (northing, flipped)
    x_var.units = "meter"
    x_var.description = "Northing coordinates in EPSG:25832"
    x_var[:] = y[::-1]  # Flip y to ensure north-up

    y_var = ds.createVariable('y', 'f4', ('y',))  # Horizontal axis (easting)
    y_var.units = "meter"
    y_var.description = "Easting coordinates in EPSG:25832"
    y_var[:] = x
